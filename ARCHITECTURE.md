...
- **Clinical hover-glossary** (`glossary.js`, `data/glossary.js` = `window.GLOSSARY`) — a
  presentation layer that lightly underlines flagged clinical terms/expressions (symptoms,
  syndromes, adverse effects, PK concepts) *everywhere prose is rendered* and defines them on
  hover; wiki-mapped terms add a "learn more" jump (`#wiki=<id>` deep-link). A MutationObserver
  auto-decorates whatever the router renders (idempotent, first-mention-per-render, skipping
  interactive controls / existing wiki-links / overlays). Complements `linkify` (which handles
  med/condition *names*); the glossary handles the *vocabulary* around them. Generated by
  `pipeline/glossary/build.js`.
...
- **XP & levels** — per correct answer; more for harder exercise types and vignettes. **XP pacing
  is deliberately slowed**: every gain flows through `PML.game.addXp(amount, kind)`, which scales by
  a global rate (0.5) and by `kind='drill'` (0.3) for Practice + Review — so progression is roughly
  half speed overall and ~30% for practice/review. `addXp` is the single lever.
