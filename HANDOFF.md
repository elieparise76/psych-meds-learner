...
- **Clinical hover-glossary (latest)**: 304 clinical words/expressions (symptoms, signs,
  syndromes, adverse effects, conditions, key PK/pharmacology concepts) are **lightly
  dotted-underlined wherever they appear in prose** across the app; hovering (or focusing) any
  flagged term pops a short definition, and terms that map to a Wiki page also show **"Click to
  learn more"** (opens that entry — a new window when the browser allows via a `#wiki=<id>`
  deep-link, else in-app). Data: `data/glossary.js` → `window.GLOSSARY`
  (`{term, aliases[], def, wiki, category}`), generated by `pipeline/glossary/build.js` from
  committed subagent extraction + a clinical-QA `overrides.json`. Engine: `app/js/glossary.js`
  (alias matcher, idempotent first-mention-per-render DOM decorator, shared tooltip, a
  MutationObserver that auto-decorates everything the router renders; skips interactive controls,
  existing wiki-links, and overlays). Toggle in Settings → "Underline clinical terms". Styles under
  "Clinical hover-glossary" in `app.css`. It is a pure presentation layer — never touches the deck
  or user state. See `pipeline/glossary/README.md`.
- **XP pacing (latest)**: progression is deliberately slowed. `PML.game.addXp(amount, kind)` scales
  every gain by a global rate (**0.5** — half) and Practice + Review gains further (**kind='drill'
  → 0.3**). So learning/quests/cram/lessons pay half, and practice/review pay ~30% of the old rate.
  All XP flows through `addXp`, so this is the one lever.
- **Tutorial skip (latest)**: the first run is still mandatory/gated (no skip, Next locked until
  Neuro finishes), but **every replay is skippable** — once `settings.tutorialSeen` is set, the
  tour shows a "Skip tour ✕" button, Escape closes it, and steps advance freely (no gate). Logic in
  `tutorial.js` (`start()` computes `skippable = tutorialSeen`).
...
to `wiki.medPage`) · `wiki` (hub + med pages + disorder pages + linkify + reverse index) · `glossary`
(clinical hover-glossary: alias matcher + DOM decorator + tooltip + auto-apply observer + wiki
deep-link) · `compare`
...
node glossary/build.js # rebuilds data/glossary.js (clinical hover-glossary) from raw/ + overrides.json
```
...
- `pipeline/glossary/` — the clinical hover-glossary. `raw/out_*.json` (committed subagent
  extraction) + `overrides.json` (clinical-QA fixes) → `data/glossary.js` via
  `node pipeline/glossary/build.js` (deterministic). See `pipeline/glossary/README.md`.
